...
# For those who insist on a "license" for this script, consider it GPL v2 licensed.
#
...
import sys
InDo_Inches = input('Do you want Threads Per Inch table?(Y/N):')
if InDo_Inches == "Y":
...
Gears_inp = input('Enter the gears you have comma delimited largest to smallest\n(for default leave blank and press enter:\nDefault:84,80,60,60,50,50,45,40,40,35,30,20,20\nDefault+27pcSet:84,80,80,72,65,60,60,57,55,54,50,50,48,45,45,40,40,35,30,30,25,20,20): ').strip() or "84,80,60,60,50,50,45,40,40,35,30,20,20"
gears_have = list(map(int,Gears_inp.split(',')))
...
# Gears included with vevor lathe
#gears_have = [84,80,60,60,50,50,45,40,40,35,30,20,20] 

# Gears included with 27 pc Vevor metal gear set ONLY
#gears_have = [80,80,72,65,60,60,57,55,54,50,50,48,45,45,40,40,35,30,25,20,20,20]

# Gears included with vevor lathe plus unique ones from the 27 pc Vevor metal gear set
#gears_have = [84,80,80,72,65,60,60,57,55,54,50,50,48,45,45,40,40,35,30,30,25,20,20]
#gears_have = [84,80,72,65,60,57,55,54,50,48,45,40,35,30,25,20]

#Gears on the lathe table only
#gears_have = [60,60,50,50,45,40,40,35,30,30]

if do_inches:
    targets = [10,11,12,13,14,16,18,20,24,27,28,32,40,44] # From tap and die set
    label = "tpi"
else:
    #targets = [0.5,0.7,0.8,1,1.5,1.75,2,2.5,3,12,13,14,15,16,18,19,20,22,24,26,28,32,36,38,40,44,48,52] # Target MM pitches
    targets = [0.5,0.7,0.8,1,1.5,1.75,2,2.5,3,12] 
    label = "mm"

No_Lathe_gears = int(input('How many total Gears does your Vevor have? (5 or the default of 4):') or "4")
if No_Lathe_gears == 5:
    leadscrew_pitch = 2
else:
    leadscrew_pitch = 1.5
...
#
#gear_ratio = gears_used(4)/54.0*gears_used(2)/gears_used(3)*gears_used(0)/gears_used(1)
#  +--+------+
#  |SS|      | 
#  |SS|      |
...
#to start the selecting gear we tell the script:
#A.) how many gears we have,
#B.) send a blank array for results
#C.) how many locations we need to pupulate and in the VEVOR Mini Metal Lathe, 8"x14" lathe it's only 4 not 5
pick_gears(gears_have, [],No_Lathe_gears)
gear_combos.sort()
